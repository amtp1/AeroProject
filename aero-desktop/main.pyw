...
class AeroLogin:
    ...
    def initialization_elememnts(self):
        ...
        self.reg_button = Button(
            self.root, text="Зарегистрироваться", bd=0, width=59, height=2, command=self.reg_user)
    ...
    def auth_user(self):
        ...
        if last_name and department and password:
            try:
                user_data = get(
                    r"http://%(host)s:%(port)i/api/user-login/%(last_name)s/%(department)s/%(password)s/" % (login_user_data))
                ...
                if not user_data["is_reg"]:
                    ...
                    if not bool(user_data["user"]):
                        ...
                        get("http://%(host)s:%(port)i/api/user-update-active/%(last_name)s/%(value)i" %
                            update_active_data)
    ...
    def remove_login_elements(self):
        ...
        self.reg_last_name_lbl.grid(
            row=0, column=0, sticky=NW, padx=190, pady=100)
        self.reg_first_name_lbl.grid(
            row=0, column=0, sticky=NW, padx=190, pady=150)
        self.reg_middle_name_lbl.grid(
            row=0, column=0, sticky=NW, padx=190, pady=200)
        self.reg_position_lbl.grid(
            row=0, column=0, sticky=NW, padx=190, pady=250)
        self.reg_department_lbl.grid(
            row=0, column=0, sticky=NW, padx=190, pady=300)
        self.reg_password_lbl.grid(
            row=0, column=0, sticky=NW, padx=190, pady=350)
        self.reg_last_name_entry.grid(
            row=0, column=0, sticky=NW, padx=370, pady=100)
        self.reg_first_name_entry.grid(
            row=0, column=0, sticky=NW, padx=370, pady=150)
        self.reg_middle_name_entry.grid(
            row=0, column=0, sticky=NW, padx=370, pady=200)
        self.reg_position_entry.grid(
            row=0, column=0, sticky=NW, padx=370, pady=250)
        self.reg_department_entry.grid(
            row=0, column=0, sticky=NW, padx=370, pady=300)
        self.reg_password_entry.grid(
            row=0, column=0, sticky=NW, padx=370, pady=350)
        self.reg_button.grid(row=0, column=0, sticky=NW,
                             padx=190, pady=400, ipadx=3)
...
class Aero:
    ...
    def initialization_elements(self):
        ...
        self.user_profile_actions.config(
            bg="black", fg="WHITE", activebackground="grey13", activeforeground="grey13", bd=0)
        self.user_profile_actions["menu"].config(
            bg="red", activebackground="red", activeforeground="black")
        self.user_profile_actions.grid(
            row=0, column=0, sticky=NW, padx=678, ipadx=30, ipady=12)
        ...
        self.fio_scrollbar = Scrollbar(
            self.root, command=self.fio_txtbox.yview, bd=0)
        ...
        self.position_scrollbar = Scrollbar(
            self.root, command=self.position_txtbox.yview, bd=0)
        self.position_txtbox.configure(
            yscrollcommand=self.position_scrollbar.set)
        ...
        self.department_scrollbar = Scrollbar(
            self.root, command=self.department_txtbox.yview, bd=0)
        self.department_txtbox.configure(
            yscrollcommand=self.department_scrollbar.set)
        ...
        self.date_scrollbar = Scrollbar(
            self.root, command=self.date_txtbox.yview, bd=0)
        ...
        self._department_scrollbar = Scrollbar(
            self.root, command=self._department_txtbox.yview, bd=0)
        self._department_txtbox.configure(
            yscrollcommand=self._department_scrollbar.set)
        ...
        self.city_scrollbar = Scrollbar(
            self.root, command=self.city_txtbox.yview, bd=0)
        ...
        self.list_employees_scrollbar = Scrollbar(
            self.root, command=self.list_employees_txtbox.yview, bd=0)
        self.list_employees_txtbox.configure(
            yscrollcommand=self.list_employees_scrollbar.set)
        ...
        self.address_scrollbar = Scrollbar(
            self.root, command=self.address_txtbox.yview, bd=0)
        self.address_txtbox.configure(
            yscrollcommand=self.address_scrollbar.set)
        ...
        # Button
        self.error_btn = Button(
            self.root, text="Отправить", bd=0, width=13, height=2,
            command=self.send_notify_error)
        ...
        self.chat_scrollbar = Scrollbar(
            self.root, command=self.chat_txtbox.yview, bd=0)
        ...
        self.calendar_scrollbar = Scrollbar(
            self.root, command=self.calendar_txtbox.yview, bd=0)
        self.calendar_txtbox.configure(
            yscrollcommand=self.calendar_scrollbar.set)

        # Calculations frame
        self.calculator_frame = Frame(self.root, bg="grey13")

        self.input_value = Entry(self.calculator_frame, font=(
            'arial', 12, 'bold'), bd=0, width=49, bg='#595954', fg='white')
        self.input_value.grid(row=0, sticky=NW, ipady=10)

        self.one_btn = Button(self.calculator_frame, text="1", **
                              self.btn_params, command=lambda: self.calculator_callback(1))
        self.one_btn.grid(row=1, sticky=NW)

        self.two_btn = Button(self.calculator_frame, text="2", **
                              self.btn_params, command=lambda: self.calculator_callback(2))
        self.two_btn.grid(row=1, sticky=NW, padx=111)

        self.three_btn = Button(self.calculator_frame, text="3", **
                                self.btn_params, command=lambda: self.calculator_callback(3))
        self.three_btn.grid(row=1, sticky=NW, padx=222)

        self.plus_btn = Button(self.calculator_frame, text="+", **
                               self.btn_params, command=lambda: self.calculator_actions("+"))
        self.plus_btn.grid(row=1, padx=332)

        self.four_btn = Button(self.calculator_frame, text="4", **
                               self.btn_params, command=lambda: self.calculator_callback(4))
        self.four_btn.grid(row=2, sticky=NW)

        self.five_btn = Button(self.calculator_frame, text="5", **
                               self.btn_params, command=lambda: self.calculator_callback(5))
        self.five_btn.grid(row=2, sticky=NW, padx=111)

        self.six_btn = Button(self.calculator_frame, text="6", **
                              self.btn_params, command=lambda: self.calculator_callback(6))
        self.six_btn.grid(row=2, sticky=NW, padx=222)

        self.sub_btn = Button(self.calculator_frame, text="-", **
                              self.btn_params, command=lambda: self.calculator_actions("-"))
        self.sub_btn.grid(row=2, padx=332)

        self.seven_btn = Button(self.calculator_frame, text="7", **
                                self.btn_params, command=lambda: self.calculator_callback(7))
        self.seven_btn.grid(row=3, sticky=NW)

        self.eight_btn = Button(self.calculator_frame, text="8", **
                                self.btn_params, command=lambda: self.calculator_callback(8))
        self.eight_btn.grid(row=3, sticky=NW, padx=111)

        self.nine_btn = Button(self.calculator_frame, text="9", **
                               self.btn_params, command=lambda: self.calculator_callback(9))
        self.nine_btn.grid(row=3, sticky=NW, padx=222)

        self.mul_btn = Button(self.calculator_frame, text="*", **
                              self.btn_params, command=lambda: self.calculator_actions("*"))
        self.mul_btn.grid(row=3, padx=332)

        self.clear_btn = Button(
            self.calculator_frame, text="C", **self.btn_params, command=self.clear_value)
        self.clear_btn.grid(row=4, sticky=NW)

        self.zero_btn = Button(self.calculator_frame, text="0", **
                               self.btn_params, command=lambda: self.calculator_callback(0))
        self.zero_btn.grid(row=4, sticky=NW, padx=111)

        self.res_btn = Button(self.calculator_frame, text="=",
                              **self.btn_params, command=self.calculator_res)
        self.res_btn.grid(row=4, sticky=NW, padx=222)

        self.div_btn = Button(self.calculator_frame, text="/", **
                              self.btn_params, command=lambda: self.calculator_actions("/"))
    ...
    def employees_page(self):
        ...
        self.position_txtbox.grid(
            row=0, column=0, sticky=NW, pady=95, padx=206)
        self.department_txtbox.grid(
            row=0, column=0, sticky=NW, pady=95, padx=414)
        self.date_txtbox.grid(row=0, column=0, sticky=NW, pady=95, padx=619)

        # Set scrolls
        self.fio_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=163, pady=95, ipady=200)
        self.position_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=369, pady=95, ipady=200)
        self.department_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=577, pady=95, ipady=200)
        self.date_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=782, pady=95, ipady=200)
    ...
    def departments_page(self):
        ...
        self.list_employees_lbl.grid(
            row=0, column=0, sticky=NW, pady=70, padx=414)
        ...
        self.list_employees_txtbox.grid(
            row=0, column=0, sticky=NW, pady=95, padx=414)
        self.address_txtbox.grid(row=0, column=0, sticky=NW, pady=95, padx=619)

        # Set scrolls
        self._department_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=163, pady=95, ipady=200)
        self.city_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=369, pady=95, ipady=200)
        self.list_employees_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=577, pady=95, ipady=200)
        self.address_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=782, pady=95, ipady=200)
    ...
    def notify_error_page(self):
        ...
        self.error_txtbox.grid(row=0, column=0, sticky=NW,
                               padx=235, pady=70, ipady=4)
    ...
    def chat_page(self):
        ...
        self.remove_calendar_elements()
        self.remove_calculator_frame()
        self.chat_txtbox.grid(row=0, column=0, sticky=NW,
                              padx=235, pady=70, ipady=32)
        self.chat_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=750, pady=70, ipady=200)
    ...
    def calendar_page(self):
        ...
        self.remove_chat_elements()
        self.remove_calculator_frame()
        self.calendar_txtbox.grid(
            row=0, column=0, sticky=NW, padx=235, pady=70, ipady=32)
        self.calendar_scrollbar.grid(
            row=0, column=0, sticky=NW, padx=750, pady=70, ipady=200)
    ...
    def calculator_page(self):
        ...
        self.calculator_frame.grid(
            row=0, column=0, sticky=NW, pady=70, padx=265)
    ...
    def send_notify_error(self):
        notify_error_text = self.error_txtbox.get("1.0", END)
        notify_error_text = re.sub("^\s+|\n|\r|\s+$", '', notify_error_text)
        if not notify_error_text:
            return messagebox.showwarning("Сообщение о проблеме", "Для отправки опишите проблему.")

        notify_error_data = {
            "host": self.host,
            "port": self.port,
            "last_name": self.last_name,
            "text": notify_error_text,
            "created": dt.strftime(dt.now(), "%Y-%m-%d %H:%M:%S.%f")
        }
        response = get("http://%(host)s:%(port)i/api/notify-error/%(last_name)s/%(text)s/%(created)s" % notify_error_data)
        response = loads(response.text)
        self.error_txtbox.delete("1.0", END)
        if not response["status"]:
                return messagebox.showerror("Ошибка", str(response["error_message"]))
        messagebox.showinfo("Сообщение о проблеме", "Сооющение успешно отправлено!")
    ...
    def actions_callback(self, action):
        if action == "Выход":
            ...
            response = get("http://%(host)s:%(port)i/api/user-update-active/%(last_name)s/%(value)i" %
                exit_user_data)
            response = loads(response.text)
            if not response["status"]:
                return messagebox.showerror("Ошибка", str(response["error_message"]))
